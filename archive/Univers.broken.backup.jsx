...
import { useParallax, ParallaxGroup } from "../tilt";
import CustomCursor from "../cursor.jsx";
import WatermarkLogo from "../components/WatermarkLogo.jsx";
...
function Sorgho({
  ...
  const stemMat = useMemo(() => new THREE.MeshStandardMaterial({ color: "#2f6e38", roughness: 0.9, metalness: 0 }), []);
  const leafMat = useMemo(() => new THREE.MeshStandardMaterial({ color: "#245d31", roughness: 0.85, metalness: 0, side: THREE.DoubleSide }), []);
  const panicleMat = useMemo(() => new THREE.MeshStandardMaterial({ color: "#d8bf67", roughness: 0.6, metalness: 0.05 }), []);

  const sphere = useMemo(() => new THREE.SphereGeometry(0.012, 10, 10), []);
  const stemGeo = useMemo(() => new THREE.CylinderGeometry(0.017, 0.024, height, 10, 1), [height]);
  ...
  useFrame(({ clock }) => {
    ...
    const sway = Math.sin(clock.elapsedTime * 0.9) * w + Math.sin(clock.elapsedTime * 1.5) * w * 0.6;
    if (group.current) {
  ...
  return (
    <group ref={group} position={[0.22, 0, 0]}>
      ...
      {leaves.map((lf, idx) => <mesh key={idx} geometry={lf.geom} material={leafMat} castShadow />)}
      <instancedMesh args={[sphere, panicleMat, panicle.positions.length]} frustumCulled={false} castShadow>
        {panicle.positions.map((p, i) => {
          const m = new THREE.Matrix4(); m.setPosition(p);
  ...
  return (
    <Html center position={[-0.25, -0.1, 0]} transform distanceFactor={1.3}>
      <div data-reveal style={{
        ...
        boxShadow: "0 0 40px rgba(0,255,170,0.10), 0 0 14px rgba(0,255,170,0.10) inset",
        ...
        <h2 style={{ color: "#7affcf", fontSize: "36px", margin: 0, textShadow: "0 0 12px rgba(0,255,170,0.35)", letterSpacing: 1 }}>
          EGOEJO
        </h2>
        <p style={{ marginTop: 12, fontSize: 20 }}>
          Ici, le don n’est <em>pas</em> une perte — c’est une énergie qui <em>circule</em>, <em>relie</em> et <em>féconde</em>.
          Chaque contribution devient un élan : une graine de lumière semée pour un monde plus humain, plus durable.
        </p>
      </div>
    </Html>
  );
...
export default function Univers() {
  const sceneRef = useParallax({ factor: 0.012 });
  const isMobile = typeof window !== "undefined" && window.matchMedia("(pointer: coarse)").matches;

  return (
    <div id="univers-root" className={isMobile ? "" : "hide-default-cursor"} style={{ width: "100%", height: "100%", background: "black" }}>
      {!isMobile && <CustomCursor />}
      <MotionToggle />
      <SoundToggle />
      <WatermarkLogo position="top-left" size={140} opacity={0.10} />
      ...
      <Canvas shadows camera={{ position: [0.5, 1.15, 2.5], fov: 45 }} gl={{ antialias: true }}>
        ...
        <Stars radius={100} depth={30} count={1000} factor={2} fade speed={0.2} />

        <ParallaxGroup refObj={sceneRef} par={0.012} />
        ...
        <Suspense fallback={<Loader />}>
          <group ref={sceneRef}>
            ...
            {/* Contenu */}
            <group position={[0, -0.4, 0]}><Sorgho /></group>
            <Card />
          </group>
        </Suspense>
