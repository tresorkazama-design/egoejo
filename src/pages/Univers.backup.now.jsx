...
import { OrbitControls, Stars, Html, Environment, SoftShadows, Billboard } from "@react-three/drei";
    ...
    <Html center position={[-0.25, -0.1, 0]} transform distanceFactor={1.3}>
      ...
      >
        ...
        <p style={{ marginTop: 12, fontSize: 20 }}>
          Ici, le don n’est <em>pas</em> une perte — c’est une énergie qui <em>circule</em>, <em>relie</em> et <em>féconde</em>. Chaque
          contribution devient un élan&nbsp;: une graine de lumière semée pour un monde plus humain, plus durable.
        </p>
      </div>
    </Html>
  );
...
export default function Univers() {
  ...
  const sceneRef = useParallax({ factor: 0.02 });
  const isMobile = typeof window !== "undefined" && window.matchMedia("(pointer: coarse)").matches;

  return (
    <div className={isMobile ? "" : "hide-default-cursor"} style={{ width: "100%", height: "100vh", background: "#000" }}>
      {!isMobile && <CustomCursor />}
      <SoundToggle />

      {/* Watermark overlay */}
      <WatermarkLogo position="top-left" size={140} opacity={0.1} />
      ...
      <Canvas shadows camera={{ position: [0.5, 1.15, 2.5], fov: 45 }} gl={{ antialias: true }}>
        ...
        {/* Parallax & contenu */}
        <ParallaxGroup refObj={sceneRef} par={0.02} />
        <group ref={sceneRef}>
          {/* Sol */}
