...
import * as THREE from "three";`r`nimport { useParallax, ParallaxGroup } from "../tilt";
import CustomCursor from "../cursor.jsx";
import WatermarkLogo from "../components/WatermarkLogo.jsx";`r`nimport { initReveal } from "../reveal";
...
/* ---------- Page ---------- */
export default function Univers() {`r`n  React.useEffect(() => initReveal(), []);`r`n  const sceneRef = useParallax({ factor: 0.02 });
  const isMobile = typeof window !== "undefined" && window.matchMedia("(pointer: coarse)").matches;

  return (
    <div className={isMobile ? "" : "hide-default-cursor"} style={{ width: "100%", height: "100%", background: "black" }}>
      ...
      <Canvas shadows camera={{ position: [0.5, 1.15, 2.5], fov: 45 }} gl={{ antialias: true }}>
        ...
        {/* Sol + contenu */}<ParallaxGroup refObj={sceneRef} par={0.02} /><group ref={sceneRef}>
