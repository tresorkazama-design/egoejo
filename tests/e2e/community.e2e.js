...
test.describe("Page Communauté", () => {
  ...
  test("affiche les fils et permet de sélectionner un fil", async ({ page }) => {
    await page.goto("/communaute");
    await expect(page.getByText(/Echanges en temps/i)).toBeVisible();
    await expect(page.getByRole("button", { name: "Fil principal" })).toBeVisible();

    await page.getByRole("button", { name: "Fil principal" }).click();
    await expect(page.getByText("Bienvenue sur le fil !")).toBeVisible();
  });
});
